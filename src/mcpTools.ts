import { MCPTool, MCPToolCall, MCPToolResult, TaskInfo, TaskExecutionResult } from './types';
...
export class MCPTools {
    ...
    async executeTool(toolCall: MCPToolCall): Promise<MCPToolResult> {
        ...
        try {
            switch (toolCall.name) {
                case 'list_tasks':
                    return await this.listTasks(toolCall.arguments as { filter?: string });
                case 'execute_task':
                    return await this.executeTask(toolCall.arguments as { taskName: string; source?: string });
                case 'get_running_tasks':
                    return await this.getRunningTasks();
                case 'terminate_task':
                    return await this.terminateTask(toolCall.arguments as { taskName: string });
                case 'get_task_details':
                    return await this.getTaskDetails(toolCall.arguments as { taskName: string; source?: string });
                default:
    ...
    private async listTasks(args: { filter?: string }): Promise<MCPToolResult> {
    ...
    private async executeTask(args: { taskName: string; source?: string }): Promise<MCPToolResult> {
        if (!args.taskName) {
    ...
    private async getRunningTasks(): Promise<MCPToolResult> {
    ...
    private async terminateTask(args: { taskName: string }): Promise<MCPToolResult> {
        if (!args.taskName) {
            throw new Error('taskName is required');
        }
    ...
    private async getTaskDetails(args: { taskName: string; source?: string }): Promise<MCPToolResult> {
        if (!args.taskName) {
            throw new Error('taskName is required');
        }
